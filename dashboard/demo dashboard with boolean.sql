...
CREATE TABLE IF NOT EXISTS `project` (
	...
	`closing_date` date default NULL, #Date, Rev Closng
	`effective_date` date default NULL, #Date, Effectv
	...
	`scale_up` boolean, #Scale-up
	`cost_overrun` boolean, #Cost Overrun
	`complex_project` boolean, #Complex Project
	`TL` varchar(100), 
	`tl_since` date default NULL, #TL Since
	`restructuring` boolean, #Restructuring
	`repeater` boolean, #Repeater Process
	`ps` varchar(100), #Proc. Specialist
	`supplemental` boolean, #Suppl Flg
